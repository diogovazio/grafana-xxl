...
RUN \
  # Hue hue hue BR
  sed -i 's|http://archive.ubuntu.com|http://br.archive.ubuntu.com|g' /etc/apt/sources.list && \
  apt-get update && \
  apt-get -y --force-yes --no-install-recommends install libfontconfig curl ca-certificates git npm && \
  ...
  pushd /tmp/grafana-zabbix && \
  npm install && \
  npm install -g grunt-cli && \
  grunt && \
  ls dist && \
  cp -r /tmp/grafana-zabbix/dist /usr/share/grafana-plugins/grafana-zabbix && \
  ...
  apt-get remove -y --force-yes curl git npm && \
